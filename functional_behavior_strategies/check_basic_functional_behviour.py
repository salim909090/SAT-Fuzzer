...
import os
import subprocess
STRATEGY_NAME = "check_basic_functional_behviour"
def run_strategy(input_path,SUT_path,seed,bugs_logs_path):
    ...
    if not is_satisfiable(output):
        log_error_case("satisfiable_cnf",file_name_full_path,SUT_path,bugs_logs_path,"satisfiable cnf result in unsatisfiable")
    ...
    if is_satisfiable(output):
        log_error_case("unsatisfiable_cnf",file_name_full_path,SUT_path,bugs_logs_path,"unsatisfiable cnf result in satisfiable")
