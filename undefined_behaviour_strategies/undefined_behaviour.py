...
def run_strategies(input_path, SUT_path, seed, bugs_logs_path):
    ...
    pathlib.Path(os.path.join(input_path, sut_name, generative_fuzzing_undefined_behviour.STRATEGY_NAME)).mkdir(
        parents=True, exist_ok=True)
    while True:
        full_input_file_path = os.path.join(input_path, sut_name, generative_fuzzing_undefined_behviour.STRATEGY_NAME,
                                            f"{counter}.cnf")
